...
contract RobinGenesisVaultTest is Test, GenesisConstants, ForkFixture {
    ...
    // system under test
    RobinGenesisVault internal vault;
    /// forge-lint: disable-next-line(mixed-case-variable)
    GenesisMarketInfo M0 = firstEligible;
    /// forge-lint: disable-next-line(mixed-case-variable)
    GenesisMarketInfo M1 = firstNonEligible;
    /// forge-lint: disable-next-line(mixed-case-variable)
    GenesisMarketInfo NEW_M = secondNonEligible;
    ...
    function setUp() public {
        _selectPolygonFork(GENESIS_FORK_BLOCK);
        ...
        vault = new RobinGenesisVault(address(ctf), address(usdc), TVL_CAP);
        ...
        emit RobinGenesisVault.MarketAdded(0, M0.conditionId, M0.yesPositionId, M0.noPositionId, M0.extraEligible);
        vault.addMarket(M0.conditionId, 600_000, M0.extraEligible, M0.collateral); // A = $0.60, B = $0.40
        // market 1: eligible = false
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.MarketAdded(1, M1.conditionId, M1.yesPositionId, M1.noPositionId, M1.extraEligible);
        ...
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.CampaignStarted(owner, BASE_REWARD, block.timestamp, block.timestamp + CAMPAIGN_DURATION);
        ...
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.PricesUpdated(block.timestamp);
    ...
    function _mintOutcome(address user, GenesisMarketInfo memory market, uint256 amount) internal {
        vm.startPrank(USDC_WHALE);
    ...
    function _deposit(address user, uint256 marketIndex, bool isA, uint256 amount) internal {
        ...
        vm.prank(user);
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.Deposit(user, marketIndex, isA, amount);
    ...
    function _withdraw(address user, uint256 marketIndex, bool isA, uint256 amount) internal {
        vm.prank(user);
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.Withdraw(user, marketIndex, isA, amount);
    ...
    function _batchDeposit(address user, uint256[] memory idxs, bool[] memory sides, uint256[] memory amts) internal {
        ...
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.BatchDeposit(user, total);
        vm.prank(user);
    ...
    function _batchWithdraw(address user, uint256[] memory idxs, bool[] memory sides, uint256[] memory amts) internal {
        ...
        for (uint256 i = 0; i < amts.length; i++) total += amts[i];
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.BatchWithdraw(user, total);
        vm.prank(user);
    ...
    function test_SetTvlCap_Emits() public {
        ...
        emit RobinGenesisVault.TvlCapUpdated(newCap, BASE_REWARD);
        vault.setTvlCap(newCap, BASE_REWARD);

        vm.expectEmit(true, true, true, true, address(vault));
        //Second check with changing base reward
        emit RobinGenesisVault.TvlCapUpdated(newCap, newBase);
    ...
    function test_Deposit_Enforces_TvlCap() public {
        ...
        vm.prank(alice);
        vm.expectRevert(RobinGenesisVault.TvlCapExceeded.selector);
    ...
    function test_BatchDeposit_LengthMismatch_Reverts() public {
        ...
        amts[1] = 1;
        vm.prank(alice);
        vm.expectRevert(RobinGenesisVault.LengthMismatch.selector);
        vault.batchDeposit(idxs, sides, amts);
    }
    ...
    function test_EndAndReplaceMarket_StopsAccrual_OnOld_AllowsNew() public {
        ...
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.MarketEnded(1);
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.MarketAdded(2, NEW_M.conditionId, NEW_M.yesPositionId, NEW_M.noPositionId, NEW_M.extraEligible);
        ...
        vm.prank(bob);
        vm.expectRevert(RobinGenesisVault.MarketNotActive.selector);
    ...
    function test_DuplicateTokenIds_Rejected() public {
        ...
        vm.expectRevert(RobinGenesisVault.DuplicateTokenId.selector);
    ...
    function test_Finalize_WithEligible_SplitsBaseAccrued_And_ExtraPool() public {
        ...
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.CampaignFinalized(
            block.timestamp,
    ...
    function test_Claim_Reverts_BeforeFinalize_And_WhenNothingToClaim() public {
        ...
        _deposit(alice, 0, true, 1_000_000);
        vm.prank(alice);
        vm.expectRevert(RobinGenesisVault.CampaignNotFinalized.selector);
        vault.claimRewards();
        ...
        vault.finalizeCampaign();
        vm.prank(carol);
        vm.expectRevert(RobinGenesisVault.ZeroAmount.selector);
        vault.claimRewards();
    }
    ...
    function test_Emergency_EnableAndBlockMutations() public {
        ...
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.EmergencyModeEnabled(block.timestamp);
        vault.enableEmergencyMode();
        assertTrue(vault.emergencyMode());

        // Owner operations blocked
        vm.expectRevert(RobinGenesisVault.InEmergency.selector);
        vault.batchUpdatePrices(prices);
    ...
    function test_EmergencyWithdrawAll_TransfersAllAndZerosBalances() public {
        ...
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.EmergencyWithdrawal(alice);
        vm.prank(alice);
    ...
    function test_EmergencyWithdrawAll_Reverts_NotInEmergency_And_ZeroAmount() public {
        ...
        vm.prank(alice);
        vm.expectRevert(RobinGenesisVault.NotInEmergency.selector);
        vault.emergencyWithdrawAll();
    ...
    function test_EmergencySweepUSDC_TransfersAllToRecipient() public {
        ...
        vault.enableEmergencyMode();
        vm.expectRevert(RobinGenesisVault.ZeroAddress.selector);
        vault.emergencySweepUsdc(address(0));

        // Sweep
        vm.expectEmit(true, true, true, true, address(vault));
        emit RobinGenesisVault.LeftoversSwept(recipient, vault.baseRewardPool());
        vault.emergencySweepUsdc(recipient);
