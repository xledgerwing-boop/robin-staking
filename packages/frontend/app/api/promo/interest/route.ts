...
export async function GET(req: NextRequest) {
    try {
        ...
        const row = await db(PROMO_INTERESTS_TABLE).where('vaultAddress', vault).sum({ total: 'totalUsd' }).first();
        const registeredUsd = (row?.total as string | null) ?? '0';
        return NextResponse.json({ vault, registeredUsd });
    } catch (e) {
        console.error('GET /api/promo/interest/oversubscription failed', e);
