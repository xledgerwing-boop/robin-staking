...
export default function VaultCapacity() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const resp = await fetch(`/api/promo/interest?vault=${VAULT}`);
    ...
    const onRegisterInterest = async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch('/api/promo/interest/register', {
                method: 'POST',
            ...
            const over = await fetch(`/api/promo/interest?vault=${VAULT}`).then(r => r.json());
